...
export const comment = (() => {
    ...
    const cancel = (id) => {
        if (document.getElementById(`form-inner-${id}`).value.length === 0 || confirm('¿Estás seguro?')) {
    ...
    const reply = (button) => {
        ...
        inner.id = `inner-${id}`;
        inner.innerHTML = `
        <label for="form-inner-${id}" class="form-label">Responder</label>
        <textarea class="form-control shadow-sm rounded-3 mb-2" id="form-inner-${id}" placeholder="Escribe un comentario"></textarea>
        <div class="d-flex flex-wrap justify-content-end align-items-center mb-0">
            ...
            <button style="font-size: 0.8rem;" onclick="comment.send(this)" data-uuid="${id}" class="btn btn-sm btn-outline-${theme.isDarkMode('light', 'dark')} rounded-3 py-0">Enviar</button>
        </div>`;
